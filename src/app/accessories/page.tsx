...
export default function AccessoriesPage() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      // Filter for accessories's products - only using type filter
      const accessoriesProducts = allProducts.filter(product =>
        product.type === 'Accessories'
      );

      // Extract unique categories from accessiores's products
      ...
      console.error('Failed to fetch accessiores\'s products:', error);
      setProducts([]);
      setTotalProducts(0);
    } finally {
      setLoading(false);
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-6 sm:py-8 mt-16 sm:mt-20">
        <h1 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6">Accessories's Collection</h1>
            ...
            {products.length > 0 ? (
              <ProductGrid
                products={products.map(p => ({
                  ...
                  type: p.type || 'Accessories',  // Add this line
                  isNew: false,
                  isFavorite: false
                }))}
                isLoading={loading}
