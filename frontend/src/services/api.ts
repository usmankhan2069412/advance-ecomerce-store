...
export const productApi = {
  ...
  async getProducts(params?: {
    ...
    try {
        ...
        if (cachedProducts && cacheTimestamp) {
          ...
          if (cacheAge < 30 * 60 * 1000) {
            try {
              ...
              if (Array.isArray(products) && products.length > 0) {
                console.log(
                  `Using ${products.length} cached products from localStorage`,
                );
                return products;
              }
            } catch (parseError) {
      ...
      if (params?.minPrice !== undefined) {
        filteredProducts = filteredProducts.filter(
          (p) => p.price >= (params.minPrice || 0),
        );
      }
      if (params?.maxPrice !== undefined) {
        filteredProducts = filteredProducts.filter(
          (p) => p.price <= (params.maxPrice || Infinity),
        );
      }
      ...
      console.log(
        `Returning ${filteredProducts.length} products after filtering`,
      );
        ...
        const emergencyProducts = [
          {
            ...
            image:
              "https://images.unsplash.com/photo-1523381210434-271e8be1f52b?w=400&q=80",
            description:
              "This is an emergency product generated when all else fails.",
  ...
  async getProductById(id: string) {
    try {
      ...
      const normalizedProduct = {
        ...response.data,
        sustainabilityScore:
          response.data.sustainabilityScore ||
          rawProduct.sustainability_score ||
          3,
      ...
      try {
        ...
        console.error(
          `Failed to fetch product ${id} from fallback:`,
          fallbackError,
        );
        return null;
      }
   ...
   */
  async createProduct(
    product: Omit<Product, "id" | "created_at" | "updated_at" | "isFavorite">,
  ) {
    try {
  ...
  async getProductSustainability(id: string) {
    try {
      const response = await apiClient.get<ProductSustainability>(
        `/products/${id}/sustainability`,
      );
      return response.data;
    } catch (error) {
