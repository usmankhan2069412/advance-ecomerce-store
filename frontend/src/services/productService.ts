...
const validateSupabase = () => {
  try {
    if (supabase && typeof supabase.from === "function") {
      ...
      console.warn(
        "Supabase client not properly initialized in productService",
      );
      return false;
    }
  } catch (error) {
...
try {
  ...
  console.error("Initial Supabase validation failed:", error);
}
...
export const productService = {
  ...
  async createProduct(product: any) {
    try {
      ...
      if (!validateSupabase()) {
        console.warn(
          "Supabase client not properly initialized in createProduct",
        );
        throw new Error("Supabase client not properly initialized");
      }
